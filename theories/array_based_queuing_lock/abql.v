(* This file contains the specification of the array-based queuing lock. *)

From iris.program_logic Require Export weakestpre.
From stdpp Require Export base orders list.
...
Section algebra.
  ...
  (* Define the operation by implementing the Op type-class *)
  ...
  Proof.
    ...
    - intros x y. unfold pcore. unfold sumRACore. intros H. inversion H.
    - intros x y. unfold pcore. unfold sumRACore. intros H. inversion H.
    - unfold pcore, sumRACore. intros x y z _ H. inversion H.
    - intros x y. destruct x, y.
      repeat rewrite sumRA_op. intros V.
      apply -> sumRA_valid in V.
      apply sumRA_valid. apply Nat.min_glb_iff in V as [Le1 Le2].
      omega.
  Qed.
...
Section abql_spec.
  ...
  Lemma know_o_exclusive γ o o' : own γ (◯ (Excl' o, GSet ∅)) -∗ own γ (◯ (Excl' o', GSet ∅)) -∗ False.
    iIntros "H1 H2".
    iDestruct (own_valid_2 with "H1 H2") as %[[] _].
  Qed.

  Lemma valid_pair (A B : cmraT) (x : A * B) : ✓ x <-> ✓ x.1 ∧ ✓ x.2.
  Proof.
    split.
    - intro.
      unfold valid, cmra_valid, prodR in H.
      unfold prod_valid in H.
      assumption.
    - intros [V1 V2].
      unfold valid. unfold cmra_valid. unfold prodR. unfold prod_valid. split; assumption.
  Qed.

  Lemma left_left_false (κ : gname) : left κ -∗ left κ -∗ False.
    iIntros "L L'".
    iDestruct (own_valid_2 with "L L'") as %H.
    iPureIntro.
    rewrite -pair_op in H.
    rewrite -Some_op in H.
    apply -> valid_pair in H.
    simpl in H.
    destruct H.
    rewrite -> (Some_valid (Excl 1%nat ⋅ Excl 1%nat)) in H.
    inversion H.
  Qed.
  ...
  Lemma right_right_false (κ : gname) : right κ -∗ right κ -∗ False.
    ...
    iPureIntro.
    rewrite -pair_op -Some_op in H.
    apply valid_pair in H as [_ H].
    simpl in *.
    apply Some_valid in H.
    inversion H.
  Qed.
  ...
  Proof.
    iIntros "P O".
    iDestruct (own_valid_2 with "O P") as %[[_ H%gset_disj_included]%prod_included Hv]%auth_both_valid.
    iFrame. iPureIntro.
    apply elem_of_subseteq_singleton in H.
    apply elem_of_set_seq in H.
    lia.
  Qed.
  ...
  Lemma invitation_cap_bound γ i cap : invitation γ i cap -∗ ⌜i ≤ cap⌝ ∗ invitation γ i cap.
  Proof.
    iIntros "I".
  ...
  Lemma invitation_split γ (i cap : nat) : ⌜0 < i⌝ -∗ invitation γ i cap -∗ invitation γ 1 cap ∗ invitation γ (i - 1) cap.
  Proof.
    iIntros "% I".
  ...
  Lemma newlock_spec (R : iProp Σ) (cap : nat) :
    {{{ R ∗ ⌜(0 < cap)%nat⌝ }}} newlock (#cap) {{{ lk γ ι κ, RET lk; (is_lock γ ι κ lk cap R) ∗ invitation ι cap cap }}}.
  Proof.
    iIntros (Φ) "[Pre %] Post".
    wp_lam.
    ...
    iMod (own_alloc (((cap, cap) : sumRA) ⋅ ((0%nat, cap) : sumRA))) as (ι) "[Hinvites HNoInvites]".
    { rewrite sumRA_op_second. rewrite Nat.add_0_r. apply (sumRA_valid cap cap). auto. }
    (* We allocate the ghost state for the lock state indicatior. *)
    iMod (own_alloc ((Excl' 1%nat, Excl' 1%nat))) as (κ) "Both". { done. }
    wp_seq. wp_alloc p as "pts".
    ...
    { iNext. rewrite /lock_inv. iExists 0%nat, 0%nat, (<[0%nat:=true]> (replicate cap false)).
      ...
      - iLeft. iFrame.
        rewrite Nat.mod_0_l. done. omega.
    }
    wp_pures.
    iApply "Post".
    unfold is_lock. iFrame.
    iExists _, _. auto.
  Qed.
  ...
  Proof.
    ...
    destruct SLeqX; try (symmetry; assumption).
    destruct (decide (cap = 0)) as [Hcap | Hcap].
    { subst. inversion LeqCap. }
    ...
    rewrite Heqr in XLeqSCap.
    rewrite ModEq in XLeqSCap.
    rewrite Heqr in H.
    rewrite ModEq in H.
    apply Zplus_lt_reg_l in H.
    apply Z.gt_lt in LeqCap.
    apply (Z.mul_lt_mono_pos_r _ _ _ LeqCap) in H.
    ...
    - apply (Z.mul_lt_mono_pos_r _ _ _ LeqCap) in XLeqSCap.
      omega.
    - rewrite Z.mul_comm.
  ...
  Lemma mod_fact (t o i cap : nat) : 0%nat < cap -> o <= t -> t < o + i -> i <= cap -> (t `mod` cap = o `mod` cap -> t = o)%nat.
  Proof.
  ...
  Lemma nth_list_with_one : forall (n a b : nat), (list_with_one n a) !! b = Some true -> a = b.
    intros n a b H.
    unfold list_with_one in H.
    apply list_lookup_insert_Some in H.
    destruct H as [[Eq] | [_ lookup]].
    - assumption.
    - apply lookup_replicate_1 in lookup as [Neq Lol]. done.
  Qed.

  Lemma wait_loop_spec γ ι κ lk cap t R :
    {{{ is_lock γ ι κ lk cap R ∗ issued γ t }}} wait_loop lk #t {{{ RET #(); locked γ κ t ∗ R }}}.
  Proof.
    iIntros (ϕ) "(Lock & Ticket) Post".
    (* Since `wait_loop` invokes itself recursively we use Löb *)
    iLöb as "IH".
    wp_lam. wp_let.
    iDestruct "Lock" as (arr nextPtr) "(-> & % & #LockInv)".
    wp_proj. wp_proj. wp_let. wp_proj. wp_op. wp_let.
    wp_bind (! (_ +ₗ _))%E.
    wp_op.
    iInv (lockN (#arr, #nextPtr, #(cap))) as (o i xs) "(>nextPts & isArr & >% & >Inv & Auth & Part)" "Close".
    unfold is_array.
    rewrite rem_mod_eq //.
    ...
    iIntros (v) "[% isArr]".
    apply list_lookup_fmap_inv in H1 as (x & -> & xEq).
    destruct x.
    - unfold issued.
      ...
      iDestruct "Part" as "[(Ho & Hr & Both & %) | [(Ticket2 & Right & %) | (Ticket2 & Left & %)]]".
      * (* The case where the lock is currently open. *)
        rewrite H3 in xEq.
        apply nth_list_with_one in xEq.
        assert (t = o) as tEqO. { destruct H1 as [L1 L2]. apply inj_lt in H. apply (mod_fact _ _ i (cap)); auto. }
        rewrite tEqO.
        ...
        { iNext. iExists o, i, (list_with_one cap (o `mod` cap)).
          ...
          rewrite H3. iFrame.
          iSplit.
          - unfold list_with_one. rewrite insert_length. rewrite replicate_length. iPureIntro. reflexivity.
          - iRight. iRight. iFrame. iPureIntro. reflexivity.
        }
      ...
      * (* The case where the lock is in the clopen state. In this state all the
        ...
        rewrite H3 in xEq.
        apply lookup_replicate_1 in xEq as [Contr _].
        inversion Contr.
      * (* The case where the lock is closed. *)
        rewrite H3 in xEq. apply nth_list_with_one in xEq.
        assert (t = o) as ->. { destruct H1 as [L1 L2]. apply inj_lt in H. apply (mod_fact _ _ i (cap)); auto. }
  ...
  Lemma acquire_spec γ ι κ lk cap R :
    {{{ is_lock γ ι κ lk cap R ∗ invitation ι 1 cap}}} acquire lk {{{ t, RET #t; locked γ κ t ∗ R }}}.
  Proof.
    ...
    { iNext. rewrite /lock_inv. iExists o, (i+1)%nat, xs. iFrame.
      iSplitL "nextPts".
      { repeat rewrite Nat2Z.inj_add. rewrite Z.add_assoc. iAssumption. }
      iSplit.
      { iPureIntro. assumption. }
      iSplitL "Invitation Invs".
      { rewrite /invitation. rewrite <- sumRA_op_second. rewrite own_op. iFrame. }
    ...
    { unfold issued. iFrame "#". iApply "Hofull". }
  ...
  Proof.
    ...
    rewrite -pair_op. rewrite right_id. rewrite left_id.
    apply auth_update.
    destruct i.
  ...
  Lemma release_spec γ ι κ lk cap o R :
    {{{ is_lock γ ι κ lk cap R ∗ locked γ κ o ∗ R }}} release lk #o {{{ RET #(); invitation ι 1 cap }}}.
  Proof.
    iIntros (H) "(#IsLock & [Locked Right] & R) Post".
    iPoseProof "IsLock" as (arr nextPtr) "(-> & % & LockInv)".
    wp_lam. wp_pures.
    ...
    { iFrame. iPureIntro. rewrite H1. apply Nat.mod_upper_bound. omega. }
    iModIntro. iIntros "psPts".
    iDestruct "Part" as "[(Locked' & _ & Both & _) | [(_ & Right' & _) | (Issued & Left & %)]]".
    ...
    (* We destruct the disjunction in the lock invariant. We know that we are in
       ...
       contradictions. *)
    iDestruct "Part" as "[(>Locked' & _ & Both & _) | [(Issued & Right & >%) | (Issued & >Left' & Eq)]]".
    * iDestruct (know_o_exclusive with "Locked Locked'") as "[]".
    ...
    * iDestruct (own_valid_2 with "Auth Locked") as
      ...
      { rewrite fmap_length. rewrite H1. apply Nat.mod_upper_bound. omega. }
      ...
      { iNext. iExists (o + 1)%nat, (i - 1)%nat, (list_with_one cap ((o + 1) `mod` cap)).
        ...
        iFrame.
        iSplitL "isArr". {
          unfold list_with_one. unfold is_array. rewrite list_fmap_insert. rewrite H2. iAssumption.
        }
        iSplit. {
          unfold list_with_one. rewrite insert_length. rewrite replicate_length. auto.
        }
        iLeft. iFrame. done.
      }
